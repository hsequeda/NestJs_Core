...
export class FindPaginatedHandler<T> implements IQueryHandler<FindPaginatedQuery> {
  constructor(private readonly repository: BaseRepository<T>) {
  }

  async execute(query: FindPaginatedQuery): Promise<IPaginatedData<T>> {

    return this.repository.findPaginated(query.filter, query.paginator, query.populate, query.select, query.sort);
